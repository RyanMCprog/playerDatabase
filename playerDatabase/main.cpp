...
int main()
{
	...
	while (ScreenClose == 0)
	{
		...
		//(checking stats, giving them each an item, reroll the stats of one party member, showing what item each is holding, and leaving)
			...
			std::cout << "What do you want to do with your party? (1) check party stats (2) give each party member an item/ replace current item (3) reroll one party members stats (4) see what item each member is holding (5) Leave" << std::endl;
			std::cin >> options;
			std::cout << std::endl;
			std::cin.clear();
			std::cin.ignore(INT_MAX, '\n');
